            ...
            <div className="text-primaryAccent font-medium">{sectionTitle}</div>
            {
                data.map(({
                    ...
                    <div
                        ...
                        className={`grid grid-cols-[25%_75%] rounded-md px-2 py-6 transition-all ${isMouseEnter[`${SECTION_ID}-${index}`] ? "bg-orange-200" : ""}`}
                        onMouseEnter={() => setIsMouseEnter({ [`${SECTION_ID}-${index}`]: (true) })}
                        onMouseLeave={() => setIsMouseEnter({ [`${SECTION_ID}-${index}`]: (false) })}
                    >
                        <div>
                            <FormattedDate isHightLight={isMouseEnter[`${SECTION_ID}-${index}`]}>{date}</FormattedDate>
